# Vault Hub 606 (Vite) - servidor local
# Duplo-clique OU cole este bloco INTEIRO no PowerShell.
...
$ProjectMarker = '606-14_VAULTCLAUDE-ImportarExportarConversas'
...
function Get-ExistingProjectServers {
    ...
    foreach ($p in $procs) {
        ...
        if ($cmdLower -notmatch 'vite|npm.*dev|bun.*dev') { continue }
        $port = $DesiredPort
...
Write-Host "Vault Hub: $Url  (Ctrl+C para parar)" -ForegroundColor Green
